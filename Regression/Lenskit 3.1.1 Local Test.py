...
model_before_2016 = svd.BiasedSVD(50, damping=5, bias=True)
model_after_2016 = svd.BiasedSVD(50, damping=5, bias=True)
...
if __name__ == '__main__':
    ...
    before_2016_train = train[train['timestamp'] <= 1588838000000]
    after_2016_train = train[train['timestamp'] > 1588838000000]
    #make a model for data before 2016 and after 2016
    model_before_2016.fit(before_2016_train)
    model_after_2016.fit(after_2016_train)
    print(type(model_after_2016))
    print(len(before_2016_train))
    print(before_2016_train)
    ...
    for i in range(len(users)):
        if timestamps[i] <= 1588838000000:
            temp = model_before_2016.predict_for_user(users[i], [items[i]])
            recs.append(temp)
        else:
            temp = model_after_2016.predict_for_user(users[i], [items[i]])
            recs.append(temp)
