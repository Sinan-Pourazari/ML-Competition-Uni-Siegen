...
def Run(Xtrain, ytrain):
    ...
    second_est =[('GRADIENT', GBmodel),
                  ('stack', clf)]
    clf2 = StackingClassifier(estimators=second_est)
    scores = stratified_cross_fold_validator_for_smote(Xtrain, ytrain, 10, clf2, num_workers=10)
    scores_no_smote = stratified_cross_fold_validator(Xtrain, ytrain, 10, clf2, num_workers=10)
    #6 8330815
    #8 8299
    ...
    clf2.fit(Xtrain,ytrain.to_numpy().flatten())
    test = pd.read_csv('test_features.csv')
    pred = clf2.predict(test[['feature_24', 'feature_16', 'feature_19', 'feature_17', 'feature_20', 'feature_30', 'feature_29', 'feature_10', 'feature_13']])
    return pred
...
if __name__ == '__main__':
    ...
    print(return_value)
    # save it as file
    return_value.to_csv('Enseble_GradienBoosters2.csv', columns=['Id', 'label'], index=False)
