...
def Run(Xtrain, ytrain):
    ...
    randomforest = RandomForestClassifier(random_state=21, n_estimators=64, criterion='entropy', min_samples_split=2,
                                          n_jobs=1, max_depth=1409,
                                          max_features='sqrt', max_leaf_nodes=138, min_samples_leaf=3, verbose=False,
                                          min_weight_fraction_leaf=0.00215566094893653)
    # make custom randomforest from whch the model starts
    RDMForest = RandomForestClassifier(random_state=211, n_estimators=20, criterion='log_loss', min_samples_split=10,
                                       n_jobs=1,
                                       min_impurity_decrease=0.0, verbose=False)
    GBmodelada = GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=10, random_state=211,
                                            verbose=False)

    ABCRDMFR = AdaBoostClassifier(estimator=RDMForest, n_estimators=200, random_state=211, learning_rate=1,
                                  algorithm='SAMME.R')
    ABCGB = AdaBoostClassifier(estimator=GBmodelada, n_estimators=200, random_state=211, learning_rate=1,
                               algorithm='SAMME.R')
